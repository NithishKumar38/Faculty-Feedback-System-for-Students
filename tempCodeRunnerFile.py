...
def form():
    if request.method == "POST":
        ...
        session['response'].extend(responses)  # Store responses
...
def submit():
    if request.method == "POST":
        if 'response' in session and session['response']:
            session['response'].insert(0, current_user.username)
            print("Session Data after storing:", session['response']) 
            sheet.append_row(session['response'])
            session.pop('response', None)
            session.modified = True  # Ensure session update
            print("Session after clearing:", session)  # Debugging
